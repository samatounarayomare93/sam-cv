...
if response.status_code == 200:
    ...
    if webhook.get('url'):
        print(f"   ⚠️ WEBHOOK ACTIVE: {webhook['url']}")
        print(f"   ⚠️ This prevents polling from working!")
        print(f"   ⚠️ Pending updates: {webhook.get('pending_update_count', 0)}")
    else:
...
if response.status_code == 200:
    ...
    if updates:
        ...
        print(f"   ⚠️ No recent updates")
else:
    print(f"   ❌ Failed: {response.text}")
print()

# 4. Delete webhook if exists
print("4️⃣ Clearing webhook (if exists)...")
...
# 5. Check commands
print("5️⃣ Checking bot commands...")
...
print("   2. Restart your bot: .sovereign_runtime/python.exe bot_full_featured.py")
