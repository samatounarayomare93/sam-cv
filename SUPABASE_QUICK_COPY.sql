-- 🔧 CHRONOS SQL SETUP - نسخ واجلس هنا في SQL Editor من Supabase
-- 📍 الموقع: https://app.supabase.com/project/lckiazbadymeikmxesit/sql/new
-- ⏱️  الوقت: ~30 ثانية
-- 📊 النتيجة: 9 جداول جديدة + سياسات أمان

-- ═══════════════════════════════════════════════════════════════════════════════
-- PROJECT CHRONOS: COMPLETE DATABASE FIX v2
-- Run this ONCE in Supabase SQL Editor to fix ALL missing tables
-- Updated: May 14, 2026
-- ═══════════════════════════════════════════════════════════════════════════════

-- 1. Create system_logs table (for dashboard live logs)
...
-- 2. Create vip_tracking table (for VIP lead tracking)
...
-- 3. Create userbot_outreach table (for Telegram outreach tracking)
...
-- 4. Verify applications table exists
...
-- 6. Verify system_settings table exists
...
-- 8. Create system_state table (used by sync_get_stats for HUD counters)
...
-- Seed initial system_state values
...
-- 9. Insert default settings if missing
INSERT INTO system_settings (key, value) VALUES ('kill_switch', 'false')
ON CONFLICT (key) DO NOTHING;

INSERT INTO system_settings (key, value) VALUES ('active_bot_leader', 'none')
ON CONFLICT (key) DO NOTHING;

INSERT INTO system_settings (key, value) VALUES ('active_bot_heartbeat', '2020-01-01T00:00:00')
ON CONFLICT (key) DO NOTHING;

INSERT INTO system_settings (key, value) VALUES ('MIN_MATCH_SCORE', '55')
ON CONFLICT (key) DO NOTHING;
